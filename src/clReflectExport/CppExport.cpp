...
#define HASH_TO_POINTER(h) ((void*)((clcpp::pointer_type)h))
    ...
    void CopyPrimitive(clcpp::Field& dest, const cldb::Field& src, clcpp::Primitive::Kind kind)
    {
        ...
        {
        case (cldb::Qualifier::VALUE):
            dest.qualifier.op = clcpp::Qualifier::VALUE;
            break;
        case (cldb::Qualifier::POINTER):
            dest.qualifier.op = clcpp::Qualifier::POINTER;
            break;
        case (cldb::Qualifier::REFERENCE):
            dest.qualifier.op = clcpp::Qualifier::REFERENCE;
            break;
        default:
    ...
    void Parent(ParentMap<PARENT_TYPE>& parents, clcpp::CArray<const CHILD_TYPE*>(PARENT_TYPE::*carray),
                clcpp::CArray<CHILD_TYPE*>& children, StackAllocator& allocator)
    {
    ...
    void BuildAttributePtrArray(CppExport& cppexp, clcpp::CArray<clcpp::Attribute*>& attributes)
    {
        ...
        int size = cppexp.db->flag_attributes.size + cppexp.db->int_attributes.size + cppexp.db->float_attributes.size +
                   cppexp.db->primitive_attributes.size + cppexp.db->text_attributes.size;
    ...
    void Link(clcpp::CArray<PARENT_TYPE>& parents, const FIELD_TYPE*(FIELD_OBJECT_TYPE::*field),
              clcpp::CArray<const CHILD_TYPE*>& children)
    {
    ...
    void Link(clcpp::CArray<PARENT_TYPE>& parents, const FIELD_TYPE* (FIELD_OBJECT_TYPE::*field)[N],
              clcpp::CArray<const CHILD_TYPE*>& children)
    {
        // Create a lookup table from hash ID to child
        typedef std::multimap<unsigned int, const CHILD_TYPE*> ChildMap;
    ...
    void BuildBaseClassArrays(CppExport& cppexp, const cldb::Database& db)
    {
        ...
        for (cldb::DBMap<cldb::TypeInheritance>::const_iterator i = db.m_TypeInheritances.begin();
             i != db.m_TypeInheritances.end(); ++i)
        {
    ...
    {
        // Allocate the array
        unsigned int nb_type_primitives =
            cppexp.db->types.size + cppexp.db->classes.size + cppexp.db->enums.size + cppexp.db->template_types.size;
    ...
    unsigned int GetFlagAttributeBits(const clcpp::CArray<const clcpp::Attribute*>& attributes)
    {
        ...
        for (unsigned int i = 0; i < attributes.size; i++)
        {
            ...
            else if (attribute.name.hash == replicate_transient_hash)
                bits |= attrFlag_ReplicateTransient;
            ...
            if (primitive.parent)
            {
                ...
                case (clcpp::Primitive::KIND_CLASS):
                    LOG(main, WARNING, "Class field '%s' within '%s' couldn't find type reference to '%s'\n", primitive.name.text,
                        primitive.parent->name.text, unresolved);
                    break;
                default:
                    break;
                }
...
// and MSVC and G++ do treat it differently. We need to provide different implementation here.
    ...
    PtrSchema& schema_enum =
        relocator.AddSchema<clcpp::Enum>(&schema_type)(&clcpp::Enum::constants, array_ofs)(&clcpp::Enum::attributes, array_ofs);

    PtrSchema& schema_field = relocator.AddSchema<clcpp::Field>(&schema_primitive)(&clcpp::Field::type)(
        &clcpp::Field::attributes, array_ofs)(&clcpp::Field::ci);
    ...
    PtrSchema& schema_namespace = relocator.AddSchema<clcpp::Namespace>(&schema_primitive)(
        &clcpp::Namespace::namespaces, array_ofs)(&clcpp::Namespace::types, array_ofs)(&clcpp::Namespace::enums, array_ofs)(
        &clcpp::Namespace::classes, array_ofs)(&clcpp::Namespace::functions, array_ofs)(&clcpp::Namespace::templates, array_ofs);
    ...
    PtrSchema& schema_primitive_attribute =
        relocator.AddSchema<clcpp::PrimitiveAttribute>(&schema_primitive)(&clcpp::PrimitiveAttribute::primitive);

    PtrSchema& schema_text_attribute = relocator.AddSchema<clcpp::TextAttribute>(&schema_primitive)(&clcpp::TextAttribute::value);
...
namespace
{
    ...
    void LogField(const clcpp::Field& field, bool name = true)
    {
        ...
        LOG_APPEND(
            cppexp, INFO, "%s",
            field.qualifier.op == clcpp::Qualifier::POINTER ? "*" : field.qualifier.op == clcpp::Qualifier::REFERENCE ? "&" : "");
